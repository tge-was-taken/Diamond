...
// The padding macro
#define INSERT_PADDING(size) \
    char CONCAT(__pad_, __LINE__)[size]

#define OFFSETOF(type, member) ((size_t)&(((type*)0)->member))

template<int TActual, int TExpected>
struct assert_offsetof
{
    static_assert(TActual == TExpected, "offsetof assertion failed");
};

template<int TActual, int TExpected>
struct assert_sizeof
{
    static_assert(TActual == TExpected, "sizeof assertion failed");
};

#define ASSERT_OFFSETOF(type, field, offset) \
    static assert_offsetof<OFFSETOF(type, field), offset> CONCAT(_, __COUNTER__)

#define ASSERT_SIZEOF(type, size) \
    static assert_sizeof<sizeof type, size> CONCAT(_, __COUNTER__)
...
#define WRITE_MEMORY_BUFFER(location, data) \
    { \
        DWORD oldProtect; \
        VirtualProtect((void*)(location), sizeof(data), PAGE_EXECUTE_READWRITE, &oldProtect); \
        memcpy((void*)(location), data, sizeof(data)); \
